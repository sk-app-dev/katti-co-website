...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  try {
    ...
    // A post's own cover image, when it has one, replaces the site-wide
    // preview card; otherwise the root opengraph-image is used.
    const cover = post.image?.asset?.url
      ? [{ url: `${post.image.asset.url}?w=1200&h=630&fit=crop&fm=jpg`, width: 1200, height: 630, alt: post.title }]
      : undefined;
    return {
      title: post.title,
      ...
      twitter: {
        ...
        title: post.title,
        description: post.excerpt,
        ...(cover && { images: cover.map((c) => c.url) }),
      },
    };
  } catch {
...
const ptComponents: PortableTextComponents = {
  block: {
    ...
    h3:         ({ children }) => <h3 style={{ fontFamily: "var(--font-brand)", fontVariantCaps: "small-caps", fontSize: "0.95rem", letterSpacing: ".12em", color: "var(--gold)", margin: "1.8rem 0 0.8rem", textTransform: "uppercase", fontWeight: 500 }}>{children}</h3>,
  ...
  return (
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 999, display: "flex", alignItems: "center", justifyContent: "flex-end", padding: "1.2rem clamp(1.5rem, 5vw, 3rem)", background: "linear-gradient(180deg, rgba(7,8,13,.99) 0%, rgba(7,8,13,.96) 100%)", borderBottom: "1px solid rgba(201,166,64,.1)", backdropFilter: "blur(12px)" }}>
        <Link href="/blog" style={{ fontFamily: "var(--font-brand)", fontVariantCaps: "small-caps", fontSize: "clamp(0.65rem, 1.2vw, 0.8rem)", letterSpacing: ".12em", textTransform: "uppercase", color: "var(--text)", textDecoration: "none", padding: "0.8rem 1.6rem", background: "linear-gradient(135deg, rgba(201,166,64,.12) 0%, rgba(201,166,64,.06) 100%)", border: "1.5px solid var(--gold)", borderRadius: "10px", transition: "all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)", fontWeight: 700, display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ↑ Back to Insights
        </Link>
      </nav>
      ...
      <div style={{ marginTop: "70px", position: "relative", width: "100%", height: "clamp(450px, 65vh, 700px)", background: "linear-gradient(135deg, rgba(201,166,64,.05) 0%, rgba(201,166,64,.01) 100%)", overflow: "hidden", display: "flex", alignItems: "flex-end", justifyContent: "flex-start" }}>
        ...
        <div style={{ position: "relative", padding: "clamp(2.5rem, 8vh, 5rem) clamp(1.5rem, 5vw, 3rem)", maxWidth: "100%", zIndex: 10 }}>
          <div style={{ fontFamily: "var(--font-brand)", fontVariantCaps: "small-caps", fontSize: "clamp(0.55rem, 1.2vw, 0.8rem)", letterSpacing: ".15em", textTransform: "uppercase", color: "var(--gold)", marginBottom: "1.4rem", fontWeight: 700, opacity: 0.95 }}>
      ...
      <main style={{ maxWidth: "clamp(700px, 85vw, 920px)", margin: "0 auto", padding: "clamp(3rem, 8vh, 5rem) clamp(1.5rem, 4vw, 3rem) clamp(4rem, 12vh, 7rem)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1.2rem", marginBottom: "3rem", paddingBottom: "2rem", borderBottom: "1px solid rgba(201,166,64,.12)", flexWrap: "wrap" }}>
          <time style={{ fontFamily: "var(--font-brand)", fontVariantCaps: "small-caps", fontSize: "clamp(0.65rem, 1.3vw, 0.8rem)", letterSpacing: ".08em", color: "var(--gold)", fontWeight: 700, textTransform: "uppercase" }}>
            {new Date(post.publishedAt).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}
          </time>
          <span style={{ width: "4px", height: "4px", background: "var(--gold)", borderRadius: "50%", opacity: 0.6 }} />
          <span style={{ fontFamily: "var(--font-brand)", fontVariantCaps: "small-caps", fontSize: "clamp(0.65rem, 1.2vw, 0.75rem)", letterSpacing: ".08em", textTransform: "uppercase", color: "var(--t3)", fontWeight: 600 }}>Katti &amp; Co.</span>
        </div>
        ...
        <div style={{ marginTop: "clamp(4rem, 12vh, 6rem)", paddingTop: "clamp(2.5rem, 8vh, 4rem)", borderTop: "1px solid rgba(201,166,64,.12)" }}>
          <div style={{ background: "linear-gradient(135deg, rgba(201,166,64,.08) 0%, rgba(201,166,64,.03) 100%)", padding: "clamp(2rem, 5vw, 3rem)", borderRadius: "16px", border: "1px solid rgba(201,166,64,.15)" }}>
            ...
            <Link href="/blog" style={{ display: "inline-flex", alignItems: "center", gap: "0.7rem", fontSize: "clamp(0.7rem, 1.3vw, 0.85rem)", letterSpacing: ".1em", textTransform: "uppercase", color: "var(--text)", textDecoration: "none", padding: "1.1rem 1.9rem", background: "linear-gradient(135deg, rgba(201,166,64,.2) 0%, rgba(201,166,64,.08) 100%)", border: "1.5px solid var(--gold)", borderRadius: "12px", transition: "all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)", fontWeight: 700, fontFamily: "var(--font-brand)", fontVariantCaps: "small-caps" }}>
